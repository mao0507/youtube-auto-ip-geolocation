...
- 推薦影片 feed 15 秒內沒載出來（網路慢、被判定為機器人、地區問題等），會自動改播一支固定的備用影片，不會直接判該帳號失敗。
- 每次失敗的嘗試都會存一張截圖到 `debug/<帳號代稱>-attempt<次數>.png`，方便你回頭看當下卡在哪個畫面。
...
4. 依序處理每個帳號（不加延遲，跑完一個馬上跑下一個）：
   ...
   2. 進 YouTube 首頁/訂閱 feed，等待推薦影片縮圖出現；15 秒內沒出現就改開固定備用影片（`dQw4w9WgXcQ`）。
   3. 有縮圖的話隨機點一支影片；走備用影片的話直接進入下一步。
